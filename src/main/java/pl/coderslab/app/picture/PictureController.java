...
public class PictureController {
    ...
    public String readAllPictures(Model model, @PathVariable("page") int page) {
        ...
        Page<Picture> pictures = pictureRepositoryPageable.findAll(new PageRequest(page - 1, 3));
        List<String> encodedPictures = pictures.stream().map(p -> p.getEncodedPic()).collect(Collectors.toList());
